...
> **امنیت کلیدها:** هیچ کلید API در کد منبع وجود ندارد. هر کاربر باید کلیدهای خودش را در فایل `.env` تنظیم کند. فایل `.env` در `.gitignore` است و هرگز commit نمی‌شود — فقط `.env.example` (بدون مقدار) در مخزن است. کلیدها در لاگ‌ها چاپ نمی‌شوند.
...
- **بودجه / دسته:** اگر بعد از تغییر بودجه یا دسته محصول دقیقی در کاتالوگ نباشد، ربات باید بگوید «محصول دقیق پیدا نشد» و پیشنهاد تغییر بودجه بدهد — **نه** اینکه محصول نامرتبط (مثلاً کیف به‌جای لپ‌تاپ) نشان دهد.
- **فالوآپ کوتاه برای ضبط ویدئو:**
  ```env
